...
def ensure_text_column(df: pd.DataFrame, text_column: str) -> pd.DataFrame:
    ...
    if "text" in df.columns:
        ...
        return df
    raise KeyError(
        f"Expected text column '{text_column}', but got columns: {df.columns.tolist()}"
    )
...
def main():
    if len(sys.argv) > 1:
        ...
        config_path = Path("ml/configs/stage3_rubert_base_classifier.yaml")
    ...
    with start_run(experiment_name=experiment_name, run_name=run_name):
        ...
        print("[INFO] Starting Stage 3 strong classifier training...")
